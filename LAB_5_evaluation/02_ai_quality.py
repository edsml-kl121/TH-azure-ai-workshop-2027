...
print("=== Relevance ===")
print(RelevanceEvaluator(model_config, credential=credential)(query=query, response=response))

# Coherence (1-5): Does the response read naturally?
print("\n=== Coherence ===")
print(CoherenceEvaluator(model_config, credential=credential)(query=query, response=response))

# Fluency (1-5): Is the response grammatically correct?
print("\n=== Fluency ===")
print(FluencyEvaluator(model_config, credential=credential)(response=response))

# Groundedness (1-5): Is the response supported by the context?
print("\n=== Groundedness ===")
print(GroundednessEvaluator(model_config, credential=credential)(response=response, context=context))

# Similarity (1-5): How close is the response to ground truth?
print("\n=== Similarity ===")
print(SimilarityEvaluator(model_config, credential=credential)(query=query, response=response, ground_truth=ground_truth))

# Retrieval (1-5): How relevant is the retrieved context?
print("\n=== Retrieval ===")
print(RetrievalEvaluator(model_config, credential=credential)(query=query, context=context))
