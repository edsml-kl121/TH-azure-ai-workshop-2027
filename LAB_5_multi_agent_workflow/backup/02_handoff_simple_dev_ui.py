...
def _handle_events(events: list[WorkflowEvent]) -> list[RequestInfoEvent]:
    ...
    seen_messages: set[tuple[str, str]] = set()  # Track messages to avoid duplicates
    ...
    for event in events:
        ...
        # RequestInfoEvent: Workflow is requesting user input
            ...
            if isinstance(event.data, HandoffAgentUserRequest):
                # Only print messages that haven't been seen (avoid duplicates with AgentRunEvent)
                _print_handoff_agent_user_request(event.data.agent_response, seen_messages)
            requests.append(event)

    return requests


def _print_handoff_agent_user_request(response: AgentResponse, seen_messages: set[tuple[str, str]]) -> None:
    ...
    Args:
        ...
        seen_messages: Set of already printed messages to avoid duplicates
    """
    ...
    # Print agent responses (only if not already printed)
