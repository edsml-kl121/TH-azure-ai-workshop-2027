...
def _handle_events(events: list[WorkflowEvent]) -> list[WorkflowEvent]:
    """Process workflow events and extract any pending user input requests.

    This function inspects each event's `.type` and:
    ...
    Returns:
        List of WorkflowEvent (type == "request_info") representing pending user input requests
    """
    requests: list[WorkflowEvent] = []
    seen_messages: set[tuple[str, str]] = set()  # Track messages to avoid duplicates

    for event in events:
        # type == "intermediate": Contains messages generated by agents during their turn
        ...
        # type == "status": Indicates workflow state changes
        ...
        # type == "output": Contains the final conversation when workflow terminates
        ...
        # type == "request_info": Workflow is requesting user input
